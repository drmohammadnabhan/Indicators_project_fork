...
import numpy as np
from scipy.stats import norm, ttest_ind, t as t_dist # Added ttest_ind, t_dist for Cycle 6
import math
import pandas as pd
...
def calculate_continuous_sample_size(baseline_mean, std_dev, mde_abs_mean, power, alpha, num_variations):
    """Calculates sample size for continuous outcomes (comparing means)."""
    if std_dev <= 0: return None, "Standard Deviation must be positive."
    if mde_abs_mean == 0: return None, "MDE for means cannot be zero." # MDE can be negative, but absolute MDE is used here
    if mde_abs_mean < 0: mde_abs_mean = abs(mde_abs_mean) # Ensure it's positive for formula
    if power <= 0 or power >= 1: return None, "Power must be > 0 and < 1."
    if alpha <= 0 or alpha >= 1: return None, "Alpha must be > 0 and < 1."
    if num_variations < 2: return None, "Num variations must be >= 2."
    ...
    # Formula for two means, equal variance assumed for simplicity in many online calculators
    # n = (2 * (std_dev**2) * (z_alpha_half + z_beta)**2) / (mde_abs_mean**2)
    # More general formula allowing different variances (though we only take one std_dev input)
    # For simplicity, common online tools often use a pooled variance or assume std_dev is for one group
    # and the other is similar, simplifying to:
    # n_per_group = 2 * (std_dev^2) * (Z_alpha/2 + Z_beta)^2 / MDE^2
    # Let's use the common simplified one assuming input std_dev is representative for both groups for variance term
...
def run_bayesian_binary_analysis(summary_stats, control_group_name, prior_alpha=1, prior_beta=1, n_samples=10000, ci_level=0.95):
    ...
    if control_group_name not in results: return None, f"Control group '{control_group_name}' not found."
    ...
    if not ordered_var_names_in_results: return results, "No variations found in results for P(Best) calculation."
    ...
    if all_samples_matrix.ndim == 2 and all_samples_matrix.shape[0] > 0 and all_samples_matrix.shape[1] == n_samples:
        for i in range(n_samples):
            ...
            best_idx_in_temp_matrix = np.argmax(current_iter_samples)
        ...
        for var_name in all_var_names:
            if var_name in results: results[var_name]['prob_best'] = 1.0 if len(all_var_names) == 1 and var_name in results else 0.0
    return results, None
...
def show_design_test_page():
    ...
    metric_type_ss = st.radio(
        "Select your primary metric type for sample size calculation:",
        ('Binary (e.g., Conversion Rate)', 'Continuous (e.g., Average Order Value)'),
        key="ss_metric_type_radio"
    )
    st.markdown("---")
    ...
    if metric_type_ss == 'Binary (e.g., Conversion Rate)':
        ...
        # ... (Binary Sample Size Calculator UI - from Cycle 3 / V0.3)
        cols_bin = st.columns(2)
        with cols_bin[0]:
            baseline_cr_percent = st.number_input(label="Baseline Conversion Rate (BCR) (%)", min_value=0.1, max_value=99.9, value=5.0, step=0.1, format="%.1f", help="Current CR of control (e.g., 5% for 5 out of 100).", key="ss_bcr_c6")
        with cols_bin[1]:
            mde_abs_percent = st.number_input(label="Minimum Detectable Effect (MDE) - Absolute (%)", min_value=0.1, max_value=50.0, value=1.0, step=0.1, format="%.1f", help="Smallest absolute CR increase to detect (e.g., 1% for 5% to 6%).", key="ss_mde_c6")
        cols2_bin = st.columns(2)
        with cols2_bin[0]:
            power_percent_bin = st.slider(label="Statistical Power (1 - \u03B2) (%)", min_value=50, max_value=99, value=80, step=1, format="%d%%", help="Probability of detecting an effect if one exists (typically 80-90%).", key="ss_power_c6")
        with cols2_bin[1]:
            alpha_percent_bin = st.slider(label="Significance Level (\u03B1) (%) - Two-sided", min_value=1, max_value=20, value=5, step=1, format="%d%%", help="Risk of false positive (typically 1-5%).", key="ss_alpha_c6")
        ...
        if st.button("Calculate Sample Size (Binary)", key="ss_calc_button_c6_bin"):
            # ... (Calculation logic and display for binary - from Cycle 3 / V0.3)
            ...
            if error_msg: st.error(error_msg)
                ...
                st.markdown(f"**Summary:** BCR: `{baseline_cr_percent:.1f}%`, MDE: `{mde_abs_percent:.1f}%` (target CR: `{target_cr_pct:.2f}%`), Power: `{power_percent_bin}%`, Alpha: `{alpha_percent_bin}%`, Variations: `{num_variations_ss_bin}`.")
                with st.expander("Show Formula Used (Binary)"):
                    ...
                    st.markdown(r"**Where:** $p_1$=BCR, $p_2=p_1+\text{MDE}$, $Z_{\alpha/2}$=Z-score for alpha, $Z_{\beta}$=Z-score for power.")

    elif metric_type_ss == 'Continuous (e.g., Average Order Value)':
        st.subheader("Sample Size Calculator (for Continuous Outcomes)")
        st.markdown("**Calculator Inputs:**")
        # --- NEW UI for Continuous Sample Size ---
        cols_cont = st.columns(2)
        with cols_cont[0]:
            baseline_mean = st.number_input(label="Baseline Mean (Control Group)", value=100.0, step=1.0, format="%.2f", help="Current average value of your metric for the control group.", key="ss_mean_c6")
        with cols_cont[1]:
            std_dev = st.number_input(label="Standard Deviation (of the metric)", value=20.0, min_value=0.1, step=0.1, format="%.2f", help="Estimated standard deviation of your continuous metric. Get from historical data if possible.", key="ss_stddev_c6")
        
        mde_abs_mean = st.number_input(label="Minimum Detectable Effect (MDE) - Absolute Mean Difference", value=5.0, step=0.1, format="%.2f", help="Smallest absolute difference in means you want to detect (e.g., $2 increase).", key="ss_mde_mean_c6")
        
        cols2_cont = st.columns(2)
        with cols2_cont[0]:
            power_percent_cont = st.slider(label="Statistical Power (1 - \u03B2) (%) ", min_value=50, max_value=99, value=80, step=1, format="%d%%", help="Typically 80-90%.", key="ss_power_cont_c6") # Added space to key
        with cols2_cont[1]:
            alpha_percent_cont = st.slider(label="Significance Level (\u03B1) (%) - Two-sided ", min_value=1, max_value=20, value=5, step=1, format="%d%%", help="Typically 1-5%.", key="ss_alpha_cont_c6") # Added space to key
        
        num_variations_ss_cont = st.number_input(label="Number of Variations (including Control) ", min_value=2, value=2, step=1, help="Total versions.", key="ss_num_var_cont_c6") # Added space to key
        st.markdown("<p style='font-size: smaller; font-style: italic;'>Note: For continuous, based on pairwise comparisons vs. control at specified α, assuming equal variances for simplicity.</p>", unsafe_allow_html=True)
        ...
        if st.button("Calculate Sample Size (Continuous)", key="ss_calc_button_c6_cont"):
            ...
            if error_msg: st.error(error_msg)
                ...
                st.markdown(f"**Summary:** Baseline Mean: `{baseline_mean:.2f}`, Est. SD: `{std_dev:.2f}`, MDE (abs diff): `{mde_abs_mean:.2f}` (target mean: `{target_mean:.2f}`), Power: `{power_percent_cont}%`, Alpha: `{alpha_percent_cont}%`, Variations: `{num_variations_ss_cont}`.")
                with st.expander("Show Formula Used (Continuous)"):
                    st.markdown("For comparing two means ($n$ per group, assuming equal variances):")
                    st.latex(r'''n = \frac{2 \cdot \sigma^2 \cdot (Z_{\alpha/2} + Z_{\beta})^2}{\text{MDE}^2}''')
                    st.markdown(r"**Where:** $\sigma$=Standard Deviation, MDE=Absolute difference in means, $Z_{\alpha/2}$=Z-score for alpha, $Z_{\beta}$=Z-score for power.")
    st.markdown("---")
    with st.expander("💡 Understanding Input Impacts on Sample Size"): # This needs update for continuous too
        st.markdown(r"""
        Adjusting input parameters affects required sample size. Understanding these trade-offs is key:
        * **Baseline Conversion Rate (BCR) / Baseline Mean:** * *Binary:* Sample size largest near BCR 50%.
            * *Continuous:* Baseline mean itself doesn't directly affect sample size formula as much as SD and MDE do, but understanding it sets context for MDE.
        ...
        * **Minimum Detectable Effect (MDE):** (Applies to both absolute difference in CRs or Means)
            ...
            * *Trade-off:* Detect smaller changes at higher cost (more samples/time).
        ...
        * **Number of Variations:** Total sample size increases proportionally.
        Balancing these is key for feasible, sound tests.
        """)
    st.markdown("---")
    st.subheader("Common Pitfalls in A/B Test Design & Execution")
    # ... (Pitfalls content from Cycle 3 - V0.3)
    pitfalls = {
...
def show_analyze_results_page():
    ...
    if 'analysis_done_c6' not in st.session_state: st.session_state.analysis_done_c6 = False # Cycle specific flag
    if 'df_analysis_c6' not in st.session_state: st.session_state.df_analysis_c6 = None
    if 'metric_type_analysis_c6' not in st.session_state: st.session_state.metric_type_analysis_c6 = 'Binary' # Default
    # ... (other session state variables from Cycle 5, potentially make them cycle specific or manage carefully)
    ...
    if uploaded_file is not None:
        try:
            ...
            with map_col1:
                st.session_state.variation_col_analysis = st.selectbox("Select 'Variation ID' column:", options=columns, index=0, key="var_col_c6")
            with map_col2:
                st.session_state.outcome_col_analysis = st.selectbox("Select 'Outcome' column:", options=columns, index=len(columns)-1 if len(columns)>1 else 0, key="out_col_c6")
            with map_col3:
                st.session_state.metric_type_analysis_c6 = st.radio(
                    "Select Metric Type for Outcome Column:", ('Binary', 'Continuous'), key="metric_type_analysis_radio_c6", horizontal=True
                )
            ...
            if st.session_state.metric_type_analysis_c6 == 'Binary':
                ...
                if st.session_state.outcome_col_analysis:
                 ...
                 st.session_state.success_value_analysis = None # Not applicable for continuous
                 # Check if outcome column is numeric for continuous
            ...
            st.session_state.alpha_for_analysis = st.slider("Significance Level (\u03B1) for Analysis (%)", 1, 10, 5, 1, key="alpha_analysis_c6") / 100.0
            ...
            if st.button(analysis_button_label, key="run_analysis_button_cycle6"):
                # Reset flags and previous results
                st.session_state.analysis_done_c6 = False
                st.session_state.freq_summary_stats_c6 = None
                st.session_state.bayesian_results_c6 = None # Reset Bayesian too, will be added for continuous in next cycle
                ...
                if valid_setup:
                    try:
                        current_df = st.session_state.df_analysis_c6.copy()
                        var_col = st.session_state.variation_col_analysis
                        out_col = st.session_state.outcome_col_analysis
                        ...
                        if st.session_state.metric_type_analysis_c6 == 'Binary':
                            ...
                            summary_stats.rename(columns={var_col: 'Variation', 'Converts': 'Conversions'}, inplace=True) # Standardize for bayesian func later
                            ...
                            metric_col_name = 'Metric Value (%)'
                            st.session_state.freq_summary_stats_c6 = summary_stats.copy() # Keep original 'Conversions' for Bayesian
                            ...
                            # Optional: Handle NaNs in outcome_processed for continuous if needed, e.g., by dropping or warning.
                            # For now, groupby().agg() will handle NaNs correctly for mean, std, count.
                            summary_stats = current_df.groupby(var_col).agg(
                                Users=(out_col, 'count'), # Count of non-NaN values
                                Mean_Value=(out_col, 'mean'),
                                Std_Dev=(out_col, 'std'),
                                Std_Err=(out_col, lambda x: x.std() / np.sqrt(x.count()) if x.count() > 0 else np.nan) # SEM
                            ).reset_index()
                            ...
                            metric_col_name = 'Mean_Value'
                            st.session_state.freq_summary_stats_c6 = summary_stats.copy()


                        # --- Run Bayesian Analysis (only if binary for now) ---
                        ...
                        st.error(f"An error occurred during data processing: {e}"); st.exception(e)
        except Exception as e:
    ...
    if st.session_state.analysis_done_c6:
        ...
        metric_type_display = st.session_state.metric_type_analysis_c6
        ...
        if summary_stats_display is not None:
            ...
            if metric_type_display == 'Binary': st.dataframe(summary_stats_display[['Variation', 'Users', 'Conversions', metric_col_name]].fillna('N/A (0 Users)'))
            else: st.dataframe(summary_stats_display[['Variation', 'Users', 'Mean_Value', 'Std_Dev', 'Std_Err']].fillna('N/A'))
            
            chart_data = summary_stats_display.set_index('Variation')[metric_col_name].fillna(0)
            ...
            if control_data_rows.empty: st.error(f"Control group data missing.")
                ...
                if metric_type_display == 'Binary':
                    # ... (Binary Frequentist Comparison - from Cycle 5)
                    ...
                    for index, row in summary_stats_display.iterrows():
                        ...
                        comparison_results_freq.append({"Variation": var_name, "Metric Value (%)": f"{var_metric_val*100:.2f}", "Absolute Uplift (%)": abs_disp, "Relative Uplift (%)": rel_disp, "P-value (vs Control)": p_val_disp, f"CI {100*(1-alpha_display):.0f}% for Diff. (%)": ci_disp, "Statistically Significant?": sig_disp})

                elif metric_type_display == 'Continuous':
                    control_users, control_mean, control_std = control_data['Users'], control_data['Mean_Value'], control_data['Std_Dev']
                    # Get data for control for t-test
                    ...
                    for index, row in summary_stats_display.iterrows():
                        var_name, var_users, var_mean, var_std = row['Variation'], row['Users'], row['Mean_Value'], row['Std_Dev']
                        if var_name == st.session_state.control_group_name_analysis: continue
                        
                        p_val_disp, ci_disp, sig_disp, abs_disp, rel_disp = 'N/A', 'N/A', 'N/A', 'N/A', 'N/A'
                        if control_users > 1 and var_users > 1: # Need >1 for std dev and t-test
                            ...
                            if len(control_group_data) < 2 or len(var_group_data) < 2:
                                sig_disp = "N/A (Too few data points for t-test in a group)"
                            else:
                                try:
                                    # Perform t-test (assuming unequal variances by default - Welch's t-test)
                                    ...
                                    s1_sq, s2_sq = var_group_data.var(ddof=1), control_group_data.var(ddof=1) # sample variances
                                    
                                    # Welch-Satterthwaite degrees of freedom (approximate)
                                    # Can be complex, for CI often use critical t from simpler df or directly use statsmodels if it provides it
                                    # For simplicity and robustness, using statsmodels' CI is better if available, but manual t for difference:
                                    pooled_se_diff = math.sqrt(s1_sq/N1 + s2_sq/N2) if N1 > 0 and N2 > 0 else 0
                                    
                                    # Degrees of freedom for Welch's t-test (can be complex to calculate manually)
                                    # For simplicity, many tools approximate with min(N1-1, N2-1) or use statsmodels result
                                    # Here, we'll use t_dist.ppf with approximate df.
                                    # A simple df for Welch's t-test when variances are unequal.
                                    # For a more robust CI, a specialized function would be better.
                                    # Using min(N1-1, N2-1) as a conservative df estimate for critical t
                                    df_t = min(N1-1, N2-1) if N1 > 1 and N2 > 1 else 1
                                    if df_t > 0 and pooled_se_diff > 0:
                                        t_crit = t_dist.ppf(1 - alpha_display / 2, df=df_t)
                                        ci_low_mean_diff = abs_diff_means - t_crit * pooled_se_diff
                                        ci_high_mean_diff = abs_diff_means + t_crit * pooled_se_diff
                                        ci_disp = f"[{ci_low_mean_diff:.3f}, {ci_high_mean_diff:.3f}]"
                                    else:
                                        ci_disp = "N/A (Cannot compute CI)"

                                except Exception as e_ttest: 
                                    p_val_disp, ci_disp, sig_disp = 'Error', 'Error', 'Error'
                                    st.warning(f"T-test error for {var_name}: {e_ttest}")

                        else: sig_disp = 'N/A (Too few users in a group)'
                        comparison_results_freq.append({"Variation": var_name, "Mean Value": f"{var_mean:.3f}", "Absolute Diff. (vs Control)": abs_disp, "Relative Diff. (%)": rel_disp, "P-value (vs Control)": p_val_disp, f"CI {100*(1-alpha_display):.0f}% for Diff.": ci_disp, "Statistically Significant?": sig_disp})
                
                if comparison_results_freq:
                    comparison_df_freq = pd.DataFrame(comparison_results_freq)
                    st.dataframe(comparison_df_freq)
                    # ... (Captions for significance - same as binary)
            ...
            # --- Display Box Plots for Continuous Data ---
            if metric_type_display == 'Continuous' and st.session_state.df_analysis_c6 is not None:
                ...
                try:
                    # Ensure outcome column is numeric before plotting
                    ...
                    if not plot_df_cont.empty:
                        ...
                        # Create a list of data series for boxplot
                        boxplot_data = [plot_df_cont[plot_df_cont[st.session_state.variation_col_analysis] == var_name][st.session_state.outcome_col_analysis] 
                                        for var_name in summary_stats_display['Variation'].unique()]
                        ax_box.boxplot(boxplot_data, labels=summary_stats_display['Variation'].unique())
                        ax_box.set_title("Outcome Distributions by Variation")
                        ax_box.set_ylabel(st.session_state.outcome_col_analysis)
                        ax_box.set_xlabel("Variation")
                        st.pyplot(fig_box)
                        plt.close(fig_box)
                    else:
                        st.caption("Not enough data to display box plots after handling non-numeric outcomes.")
                except Exception as e_plot:
                    st.warning(f"Could not generate box plots: {e_plot}")


        # --- Bayesian Results Section (currently only for Binary) ---
        if metric_type_display == 'Binary':
            st.markdown("---"); st.subheader("Bayesian Analysis Results (Binary Outcome)")
            if st.session_state.bayesian_results_c6:
                # ... (Bayesian display logic from Cycle 5 - V0.5.1)
                ...
                for var_name, b_res in st.session_state.bayesian_results_c6.items():
                    ...
                    bayesian_data_to_display.append({"Variation": var_name, "Posterior Mean CR (%)": f"{b_res['mean_cr']*100:.2f}", f"{100*(1-alpha_display):.0f}% CrI for CR (%)": f"[{b_res['cr_ci_low']*100:.2f}, {b_res['cr_ci_high']*100:.2f}]", "P(Better > Control) (%)": prob_better_html, "Expected Uplift (abs %)": f"{b_res.get('expected_uplift_abs', 0)*100:.2f}" if b_res.get('expected_uplift_abs') is not None else "N/A (Control)", f"{100*(1-alpha_display):.0f}% CrI for Uplift (abs %)": cri_uplift_html, "P(Being Best) (%)": f"{b_res.get('prob_best',0)*100:.2f}"}) # Added .get for prob_best
                ...
                for var_name, b_res in st.session_state.bayesian_results_c6.items():
                    posterior = beta_dist.pdf(x_cr, b_res['alpha_post'], b_res['beta_post']); ax_cr.plot(x_cr, posterior, label=f"{var_name} (Post. α={b_res['alpha_post']:.1f}, β={b_res['beta_post']:.1f})"); ax_cr.fill_between(x_cr, posterior, alpha=0.2)
                ...
                num_vars_to_plot = sum(1 for var_name in st.session_state.bayesian_results_c6 if var_name != control_group_name_for_bayesian and st.session_state.bayesian_results_c6[var_name].get('diff_samples_vs_control') is not None)
                if num_vars_to_plot > 0:
                    cols_diff_plots = st.columns(min(num_vars_to_plot, 3)); col_idx = 0
                    for var_name, b_res in st.session_state.bayesian_results_c6.items():
                        if var_name == control_group_name_for_bayesian or b_res.get('diff_samples_vs_control') is None: continue
                        with cols_diff_plots[col_idx % min(num_vars_to_plot, 3)]:
                            ...
                            ax_diff.axvline(0, color='grey', linestyle='--'); ax_diff.axvline(b_res['expected_uplift_abs'], color='red', linestyle=':', label=f"Mean Diff: {b_res['expected_uplift_abs']*100:.2f}%")
                            ...
                            col_idx +=1
                st.markdown("""**Interpreting Bayesian Results (Briefly):** ... (text from V0.5.1)""")
            else: st.info("Bayesian analysis results are not available (currently only for Binary outcomes).")
...
st.sidebar.markdown("---")
st.sidebar.info("A/B Testing Guide & Analyzer | V0.6 (Cycle 6)")
