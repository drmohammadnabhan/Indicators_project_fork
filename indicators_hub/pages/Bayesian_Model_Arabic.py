...
# You might need to install a font like 'Amiri', 'KacstOne', or ensure 'Arial'/'Tahoma' on your system supports Arabic.
# Streamlit sharing might have limited font support, so test thoroughly there.
try:
    # For local development, you can specify a path to a .ttf font file
    # font_path = "path/to/your/arabic_font.ttf"
    # arabic_font = FontProperties(fname=font_path)
    # plt.rcParams['font.family'] = arabic_font.get_name()

    # Common fonts that might support Arabic (less reliable across all systems)
    plt.rcParams['font.family'] = 'sans-serif' # General fallback
    plt.rcParams['font.sans-serif'] = ['Tahoma', 'DejaVu Sans', 'Arial', 'Amiri', 'KacstOne'] # Prioritize Arabic-friendly fonts
...
translations = {
    ...
    "ar": {
        ...
        "introduction_objectives_markdown_1": """
        يحدد هذا المقترح إطار **تقدير Bayesian التكيفي** المصمم لتعزيز عملية جمع وتحليل بيانات الاستطلاع المتعلقة برضا حجاج بيت الله الحرام وتقييم الخدمات المقدمة من قبل مختلف الشركات.
        ...
        * **الاختبار التجريبي الشامل:** قبل التنفيذ على نطاق كامل، قم بإجراء دراسات تجريبية شاملة لتحسين العملية واختبار التكنولوجيا وتحديد التحديات غير المتوقعة.
        ...
        "interactive_illustration_prior_alpha_label": "Prior Alpha (α₀)", # Kept as is per request for sliders
        "interactive_illustration_prior_beta_label": "Prior Beta (β₀)",   # Kept as is per request for sliders
        ...
        "interactive_illustration_initial_prior_alpha_discount_label": "Initial Prior Alpha (لفترة جديدة إذا كان الخصم كبيرًا)", # Kept for slider
        "interactive_illustration_initial_prior_beta_discount_label": "Initial Prior Beta (لفترة جديدة إذا كان الخصم كبيرًا)",   # Kept for slider
...
def introduction_objectives(lang='en'):
    ...
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['introduction_objectives_markdown_1']}</div>", unsafe_allow_html=True)
    st.subheader(translations[lang]["introduction_objectives_subheader_1.1"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['introduction_objectives_markdown_1.1_content']}</div>", unsafe_allow_html=True)

def challenges_addressed(lang='en'):
    st.header(translations[lang]["challenges_addressed_header"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['challenges_addressed_markdown']}</div>", unsafe_allow_html=True)

def bayesian_adaptive_methodology(lang='en'):
    st.header(translations[lang]["bayesian_adaptive_methodology_header"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['bayesian_adaptive_methodology_markdown_intro']}</div>", unsafe_allow_html=True)

    st.subheader(translations[lang]["bayesian_adaptive_methodology_subheader_3.1"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['bayesian_adaptive_methodology_markdown_3.1_content']}</div>", unsafe_allow_html=True)

    st.subheader(translations[lang]["bayesian_adaptive_methodology_subheader_3.2"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['bayesian_adaptive_methodology_markdown_3.2_content']}</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['bayesian_adaptive_methodology_markdown_3.3_content']}</div>", unsafe_allow_html=True)

    st.subheader(translations[lang]["bayesian_adaptive_methodology_subheader_3.4"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['bayesian_adaptive_methodology_markdown_3.4_content']}</div>", unsafe_allow_html=True)

    st.subheader(translations[lang]["bayesian_adaptive_methodology_subheader_3.5"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['bayesian_adaptive_methodology_markdown_3.5_content']}</div>", unsafe_allow_html=True)

def implementation_roadmap(lang='en'):
    st.header(translations[lang]["implementation_roadmap_header"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['implementation_roadmap_markdown']}</div>", unsafe_allow_html=True)
...
def note_to_practitioners(lang='en'):
    ...
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['note_to_practitioners_markdown_5.1_content']}</div>", unsafe_allow_html=True)

    st.subheader(translations[lang]["note_to_practitioners_subheader_5.2"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['note_to_practitioners_markdown_5.2_content']}</div>", unsafe_allow_html=True)

    st.subheader(translations[lang]["note_to_practitioners_subheader_5.3"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['note_to_practitioners_markdown_5.3_content']}</div>", unsafe_allow_html=True)

    st.subheader(translations[lang]["note_to_practitioners_subheader_5.4"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['note_to_practitioners_markdown_5.4_content']}</div>", unsafe_allow_html=True)

def interactive_illustration(lang='en'):
    st.header(translations[lang]["interactive_illustration_header"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['interactive_illustration_markdown_intro']}</div>", unsafe_allow_html=True)

    st.markdown("---")
    col1, col2 = st.columns(2)

    # Use a unique suffix for keys based on language to avoid widget state issues
    key_suffix = "_" + lang

    with col1:
        st.subheader(translations[lang]["interactive_illustration_prior_beliefs_header"])
        st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['interactive_illustration_prior_beliefs_markdown']}</div>", unsafe_allow_html=True)
        # Slider labels are kept in English as per implicit request for "technical terms"
        prior_alpha = st.slider("Prior Alpha (α₀)", min_value=0.1, max_value=50.0, value=1.0, step=0.1, key="prior_a" + key_suffix)
        prior_beta = st.slider("Prior Beta (β₀)", min_value=0.1, max_value=50.0, value=1.0, step=0.1, key="prior_b" + key_suffix)
        ...
        if prior_alpha > 0 and prior_beta > 0:
            ...
            prior_ci = (0,0) # avoid errors later


    with col2:
        st.subheader(translations[lang]["interactive_illustration_new_data_header"])
        st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['interactive_illustration_new_data_markdown']}</div>", unsafe_allow_html=True)
    ...
    if prior_alpha > 0 and prior_beta > 0 : # Check if prior was valid
        posterior_alpha, posterior_beta = update_beta_parameters(prior_alpha, prior_beta, num_satisfied, num_not_satisfied)
        # The .format part needs to be present in the translated string as well
        posterior_markdown_text = translations[lang]["interactive_illustration_posterior_markdown"]
        if "{posterior_alpha:.1f}" in posterior_markdown_text and "{posterior_beta:.1f}" in posterior_markdown_text:
             st.markdown(posterior_markdown_text.format(posterior_alpha=posterior_alpha, posterior_beta=posterior_beta), unsafe_allow_html=True)
        else: # Fallback if placeholders are missing in translation
             st.markdown(f"The posterior distribution is $Beta(\\alpha_0 + k, \\beta_0 + n - k) = Beta({posterior_alpha:.1f}, {posterior_beta:.1f})$", unsafe_allow_html=True)
        ...
        # Ensure current_width is valid before comparison
        if posterior_alpha <=0 or posterior_beta <=0: #CI would be (0,0), width 0
            current_width = float('inf') # Make it fail the precision test

        if current_width <= target_width and current_width > 0 : # current_width > 0 to avoid (0,0) CI being "precise"
        ...
        posterior_alpha, posterior_beta = 1.0, 1.0 # Dummy values for next section

    st.markdown("---")
    st.subheader(translations[lang]["interactive_illustration_discounting_header"])
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['interactive_illustration_discounting_markdown']}</div>", unsafe_allow_html=True)
    ...
    # Slider labels for initial prior alpha/beta in discount section kept as per request
    initial_prior_alpha_discount = st.number_input("Initial Prior Alpha (for new period if discounting heavily)", min_value=0.1, value=1.0, step=0.1, key="init_prior_a_disc" + key_suffix)
    initial_prior_beta_discount = st.number_input("Initial Prior Beta (for new period if discounting heavily)", min_value=0.1, value=1.0, step=0.1, key="init_prior_b_disc" + key_suffix)
    ...
    if "{new_prior_alpha:.2f}" in new_prior_text_template and "{new_prior_beta:.2f}" in new_prior_text_template:
...
def conclusion(lang='en'):
    ...
    st.markdown(f"<div dir='{'rtl' if lang=='ar' else 'ltr'}'>{translations[lang]['conclusion_markdown']}</div>", unsafe_allow_html=True)

# --- Streamlit App Structure ---

# Language selection
language_options = {"English": "en", "العربية": "ar"}
selected_language_display = st.sidebar.radio("Language / اللغة", list(language_options.keys()), index=0) # Default to English
...
if current_lang == 'ar':
    st.markdown("""
        <style>
            /* General RTL for body */
            body { direction: rtl !important; text-align: right !important; }
            /* Sidebar content */
            .st-emotion-cache-1cypcdb { direction: rtl !important; text-align: right !important; } /* Adjust class if Streamlit updates */
            .stRadio > label { /* Ensure radio labels in sidebar are RTL */
                display: flex;
                flex-direction: row-reverse;
                margin-left: 5px; /* Adjust spacing as needed */
                justify-content: flex-end; /* Align radio button to the right of label */
            }
            .stRadio > label > div:first-child { /* The radio button itself */
                 margin-right: 0 !important; /* Remove potential left margin */
                 margin-left: 0.5rem; /* Add some space between button and text */
            }
             /* Main content headers */
            h1, h2, h3, h4, h5, h6 { text-align: right !important; }
            /* Ensure markdown content within divs is RTL */
            div[dir="rtl"] * { text-align: right !important; }
            /* Dataframe text alignment */
            .dataframe th, .dataframe td { text-align: right !important; direction: rtl !important; }
            /* Slider labels - if they need specific RTL handling and aren't covered by parent */
            /* stSlider label might be tricky, often inherits from parent */
            /* Input labels */
            .stTextInput label, .stNumberInput label { text-align: right !important; display: block; }
            /* Fix for Streamlit buttons in RTL */
...
# Define page dictionaries using original English keys for functions
# but display translated keys in the radio button
PAGES_FUNCTIONS = {
...
# Get the display names for the current language
page_display_names = [translations[current_lang]["sections"][en_key] for en_key in PAGES_FUNCTIONS.keys()]
# Create a mapping from display name back to English key
...
selected_display_name = st.sidebar.radio(translations[current_lang]["go_to"], page_display_names)

# Get the corresponding English key to call the function
...
st.sidebar.markdown("---")
st.sidebar.info(
    translations[current_lang]["sidebar_info"]
)
